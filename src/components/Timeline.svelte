<script lang="ts">
  ...
  export let onAddZoomForClick:
    | ((clickEvent: PointerEventRecord, seconds?: number) => void)
    | null = null;
  export let onSeek: ((seconds: number) => void) | null = null;
  export let segments: RecordingSegment[] = [];
  export let onSegmentTrimChange:
    | ((segmentId: string, edge: "start" | "end", valueMs: number) => void)
    | null = null;
  ...
  $: clickMarkers =
    segmentBoundaries.length > 0
      ? segmentBoundaries.flatMap((b) => {
          ...
          if (!seg) return [];
          return getPointerRecords(seg.events)
  ...
  $: timelineEvents = segmentBoundaries.flatMap((b) =>
    ($timelineStore.segmentEvents?.[b.id] ?? []).map((event) => ({
      segmentId: b.id,
      event,
    }))
  );

  let optimisticSegmentTrim: {
    id: string;
    trimStartSec: number;
    trimEndSec: number;
  } | null = null;
  ...
  const handleTrackClick = (e: MouseEvent) => {
    ...
    const targetSegment = segmentBoundaries.find(
      (b) => seconds >= b.originalStartSec && seconds <= b.originalEndSec
    );
    if (!targetSegment) return;

    const segmentLocalStart = Math.max(
      0,
      seconds - targetSegment.originalStartSec
    );
  ...
  const handlePointerMove = (e: PointerEvent) => {
    ...
    if (dragState.type === "segment-trim") {
      ...
      if (dragState.edge === "start") {
        newTrimStart = Math.max(
          0,
          Math.min(
            b.originalDurationSec - b.trimEndSec - 0.1,
            dragState.initialTime + deltaTime
          )
        );
      } else {
        newTrimEnd = Math.max(
          0,
          Math.min(
            b.originalDurationSec - b.trimStartSec - 0.1,
            dragState.initialTime - deltaTime
          )
        );
      }
    ...
    if (dragState.id) {
      ...
      if (dragState.type === "event-move") {
        ...
        optimisticEvent = {
          id: dragState.id,
          startTime: dragState.initialTime,
          duration: Math.max(0.1, dragState.initialDuration! + deltaTime),
        };
        ...
        optimisticEvent = {
          ...
          duration: Math.max(0.1, dragState.initialDuration! - actualDelta),
        };
      }
    }
  };
  ...
  const stopDragging = () => {
    if (
      ...
      const val =
        edge === "start"
          ? optimisticSegmentTrim.trimStartSec
          : optimisticSegmentTrim.trimEndSec;
  ...
  const handleEventPointerDown = (
    ...
    e.stopPropagation();
    if (!trackEl) return;
    const segmentId = segmentBoundaries.find((b) =>
      ($timelineStore.segmentEvents?.[b.id] ?? []).some((e) => e.id === eventId)
    )?.id;
    if (!segmentId) return;
    const targetEvent = ($timelineStore.segmentEvents?.[segmentId] ?? []).find(
      (e) => e.id === eventId
    );
  ...
  const handleEventClick = (eventId: string, event: MouseEvent) => {
    ...
    if (!dragState) {
      ...
      suppressedClickEventId = null;
      const segmentId = segmentBoundaries.find((b) =>
        ($timelineStore.segmentEvents?.[b.id] ?? []).some(
          (e) => e.id === eventId
        )
      )?.id;
      if (!segmentId) return;
  ...
  const handleMarkerClick = (
    index: number,
    marker: { event: PointerEventRecord; seconds: number }
  ) => {
    const clickZoomEvent = segmentBoundaries
      .flatMap((b) =>
        ($timelineStore.segmentEvents?.[b.id] ?? []).map((e) => ({
          boundary: b,
          event: e,
        }))
      )
  ...
  <TimelineToolbar
    ...
    onDeleteSelected={() => {
      if ($timelineStore.selectedEvent) {
        timelineStore.deleteEvent(
          $timelineStore.selectedEvent.segmentId,
          $timelineStore.selectedEvent.eventId
        );
      }
    }}
    onReset={handleReset}
  ...
  >
    ...
    <div class="timeline-track-inner">
      ...
      <TimelineCurrentTimeIndicator
        {duration}
        position={effectiveToDisplayTime(
          currentTime,
          duration,
          segmentBoundaries
        )}
